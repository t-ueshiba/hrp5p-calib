...
#define BottomMargin(w) (int)(w->text.Margin - 1)
...
Initialize(Widget treq, Widget tnew, ArgList args, Cardinal * num)
{
  ...
  new->text.Margin += new->threeD.shadow_width;
...
Redisplay(Widget aw, XExposeEvent * event, Region region)
{
  ...
  TextFieldWidgetClass wclass = (TextFieldWidgetClass) XtClass(w);
...
DrawTextReposition(TextFieldWidget w)
{
  ...
  if (width > 0) {
    ...
    XCopyArea(XtDisplay(w), XtWindow(w), XtWindow(w),
      ...
      (unsigned int) width, (unsigned int) w->core.height - 2*w->threeD.shadow_width,
...
DrawTextWithCopyArea(TextFieldWidget w)
{
  ...
  if (w->text.TextMaxLen > 0) {
    ...
    XClearArea(XtDisplay(w), XtWindow(w),
      w->text.Margin + xsrc, w->threeD.shadow_width,
      (unsigned int) width, w->core.height - 2*w->threeD.shadow_width, False);
  }
  else if (w->text.TextWidth < w->text.OldTextWidth) {
    XClearArea(XtDisplay(w), XtWindow(w),
      w->text.Margin + w->text.XOffset + w->text.TextWidth, w->threeD.shadow_width,
      w->text.OldTextWidth - w->text.TextWidth + 1,
      w->core.height - 2*w->threeD.shadow_width, False);
  }
